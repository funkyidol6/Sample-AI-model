...
def index(val_found:bool=True):
    ...
    if request.method == "POST":
        if request.files.get('csv') is None or request.files['csv'].filename == '' or float(request.form.get('ratio')) >= 1 or int(request.form.get('n_estimator')) not in range(10,1000) or request.form.get('finding_col') == '':
            val_found = False
        else:
